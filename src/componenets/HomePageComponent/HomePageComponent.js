  ...
  const submit_form = async (fields) => {
    ...
    try {
      const responseData = await sendRequest(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/cars/${PostedCar.data[0]._id}`,
        "PUT",
        JSON.stringify({
        ...
        <div>
          {ScannedQr === PostedCar?.data[0].qr_str ? (
            <Container className="update_maint_date_container">
              <div style={{ width: "500px", margin: "auto", maxWidth: "90%" }}>
                <SubmitComponent
                  PostedCar={PostedCar}
              ...
              <Row>
                <Col xs="12" md="4">
                  ...
                  <div className="page_sub_title">
                    {" "}
                    Enter car's plate info then scan the QR code to update the
